...
void writeFile(string outPath, vector<T> month, vector<T> repeatRes)
{
    ...
    for (int i = 0; i < 12; i++)
    {
        double monthAverage =  double(month[i]) / double(repeatRes[i]);
        outfile << repeatRes[i] << " "
                << month[i] << " "
                << monthAverage << endl;
        ;
...
int main()
{
    string outPath = "result_1.4.txt";
    // vector<string> pathname = {"1.txt", "2.txt", "3.txt", "4.txt", "5.txt", "6.txt", "7.txt", "8.txt",
    //                            "9.txt", "10.txt", "11.txt", "12.txt", "13.txt", "14.txt", "15.txt"};
    vector<string> pathname = {"16.txt", "17.txt", "18.txt","19.txt","20.txt"};
